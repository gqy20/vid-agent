...
- 分段预览默认覆盖到 `remotion/renders/git-course/<episode-id>/renders/current/scenes/`，文件名必须带顺序号和 scene id，例如 `01_hook.mp4`、`02_bad-model.mp4`。不要每次修改都新建带日期或描述词的 mp4 输出目录；只有用户明确要求版本对比时才另存候选版本。
