...
touch "$TMP_ROOT/run.log"   # docker -v 文件挂载：宿主必须先有文件，否则建成目录
...
AUTH_CAST="$CAST" AUTH_LOG="$TMP_ROOT/run.log" python3 - <<'PY'
import os
...
if [[ -f "$POSTPROCESS" ]]; then
  ...
  python3 "$POSTPROCESS" --cast "$CAST" --input "$BASE_VIDEO" --output "$MASKED_VIDEO" \
    --secret-length "${#ANTHROPIC_AUTH_TOKEN}"
  mv "$MASKED_VIDEO" "$OUT"
else
...
printf '{\n  "id": "%s",\n  "durationInFrames": %s,\n  "holdFromFrame": %s,\n  "width": %s,\n  "height": %s,\n  "fps": 30,\n  "font": "JetBrainsMono Nerd Font Mono",\n  "fontSize": 24,\n  "mode": "%s",\n  "theme": "claude-code-termius-dark"\n}\n' \
  "$EP" "$FRAMES" "$HOLD_FROM" "$W" "$H" "$MODE" > "$METADATA"
