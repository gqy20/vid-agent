...
if [[ "$PROJECT" == "git-course-lab" && "$RECORDING_ID" =~ ^ep(3[3-9]|40)- ]]; then
...
if [[ "$PROJECT" == "git-course-lab" && "$RECORDING_ID" =~ ^ep(49|5[0-6])- ]]; then
  [[ -f "$DEMO" ]] || DEMO="$DEMO_DIR/_ep49_56_demos.sh"
  [[ -f "$FIXTURE" ]] || FIXTURE="$BASE/fixtures/_ep49_56_workflows.sh"
fi
if [[ "$PROJECT" == "git-course-lab" && "$RECORDING_ID" =~ ^ep(5[7-9]|6[0-4])- ]]; then
  [[ -f "$DEMO" ]] || DEMO="$DEMO_DIR/_ep57_64_demos.sh"
  [[ -f "$FIXTURE" ]] || FIXTURE="$BASE/fixtures/_ep57_64_workflows.sh"
fi
...
asciinema rec --quiet --headless --overwrite --return --window-size 72x14 \
  --command "TERMINAL_RECORDING_ID='$RECORDING_ID' TERMINAL_RECORDING_WORKDIR='$WORKDIR' HOME='$HOME_DIR' bash '$DEMO'" "$CAST"
