...
def transcode(raw_webm: Path, output_mp4: Path) -> None:
    subprocess.run(
        [
            "ffmpeg",
            "-hide_banner",
...
def main() -> int:
    ...
    except (FileNotFoundError, KeyError, RuntimeError, subprocess.CalledProcessError, ValueError) as exc:
