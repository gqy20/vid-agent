...
def label(text: str, size: int = 30, color: str = TEXT_PRIMARY, weight: str = "MEDIUM") -> Text:
...
def object_box(kind: str, detail: str, color: str, width: float = 2.7, height: float = 1.2) -> VGroup:
    ...
    subtitle = label(detail, size=19, color=TEXT_SECONDARY)
...
def small_field(
    ...
    k = label(key, size=key_size, color=color, weight="BOLD").move_to(box.get_left() + RIGHT * 0.52)
    v = label(value, size=value_size, color=TEXT_PRIMARY).move_to(box.get_right() + LEFT * 0.95)
...
def commit_node(name: str, color: str = TEXT_PRIMARY, radius: float = 0.44) -> VGroup:
    ...
    txt = label(name, size=30, color=TEXT_PRIMARY, weight="BOLD").move_to(circle.get_center())
...
def pulse_ring(center, color: str, radius: float = 0.75) -> Circle:
    return Circle(radius=radius, color=color, stroke_width=10, stroke_opacity=0.22).move_to(center)
...
class Ep03ObjectModel(Scene):
    ...
    def construct(self):
        ...
        header = label("对象不是文件夹，是引用关系", size=44, color=TEXT_PRIMARY, weight="BOLD")
        ...
        entry_a = small_field("app.js", "blob a1f3", color=GIT_HEAD, width=3.55, key_size=17, value_size=17).move_to(
            tree_box.get_center() + DOWN * 0.28
        )
        entry_b = small_field("search.js", "blob 9c2e", color=GIT_HEAD, width=3.55, key_size=17, value_size=17).move_to(
        ...
        commit_tree = small_field("tree", "7b4d...", color=GIT_MAIN, width=3.2).move_to(commit_box.get_center() + DOWN * 0.28)
        ...
        tree_pulse = pulse_ring(tree_box.get_center(), GIT_HEAD, radius=1.35)
        commit_pulse = pulse_ring(commit_box.get_center(), GIT_MAIN, radius=1.25)

        final_note = label("commit 只保存引用；真正的内容在 blob 里。", size=30, color=TEXT_PRIMARY)
        ...
        self.play(FadeIn(tree_pulse, scale=0.6), tree_box.animate.set_stroke(color=GIT_HEAD, width=8), run_time=0.8)
        self.play(FadeOut(tree_pulse, scale=1.35), run_time=0.4)
        self.wait(2.3)
        ...
        self.play(FadeIn(commit_pulse, scale=0.6), run_time=0.35)
        self.play(
        ...
        self.play(FadeOut(commit_pulse, scale=1.35), run_time=0.4)
        self.wait(2.2)
        self.play(FadeIn(final_note, shift=UP * 0.12), run_time=0.8)
        self.wait(2.4)
...
class Ep03ParentChain(Scene):
    ...
    def construct(self):
        ...
        title = label("parent 指向过去", size=52, color=TEXT_PRIMARY, weight="BOLD").to_edge(UP, buff=0.5)
        c0 = commit_node("C0", color=TEXT_SECONDARY, radius=0.58).move_to([-4.7, -0.1, 0])
        c1 = commit_node("C1", color=TEXT_SECONDARY, radius=0.58).move_to([0.0, -0.1, 0])
        c2 = commit_node("C2", color=TEXT_PRIMARY, radius=0.58).move_to([4.7, -0.1, 0])
        ...
        meta = VGroup(
            small_field("tree", "7b4d...", color=GIT_MAIN, width=3.65),
        ...
        note = label("parent 把历史串成链。", size=31, color=TEXT_PRIMARY, weight="BOLD").move_to([-1.4, -3.1, 0])
        c2_pulse = pulse_ring(c2.get_center(), GIT_MAIN, radius=0.9)
        c1_pulse = pulse_ring(c1.get_center(), GIT_FEATURE, radius=0.9)
        ...
        self.play(FadeIn(c2_pulse, scale=0.65), c2[0].animate.set_stroke(color=GIT_MAIN, width=8), run_time=0.8)
        self.play(FadeOut(c2_pulse, scale=1.45), run_time=0.4)
        self.wait(3.2)
        ...
        self.play(FadeIn(c1_pulse, scale=0.65), run_time=0.35)
        self.play(FadeOut(c1_pulse, scale=1.45), run_time=0.35)
        self.wait(2.5)
...
class Ep03HashIdentity(Scene):
    ...
    def construct(self):
        ...
        title = label("hash 来自 commit 的所有字段", size=50, color=TEXT_PRIMARY, weight="BOLD")
        title.to_edge(UP, buff=0.5)

        fields = VGroup(
            small_field("tree", "7b4d...", color=GIT_MAIN, width=4.2, key_size=19, value_size=19),
            small_field("parent", "C1", color=GIT_FEATURE, width=4.2, key_size=19, value_size=19),
            small_field("author", "Lin", color=TEXT_SECONDARY, width=4.2, key_size=19, value_size=19),
            small_field("time", "10:20", color=TEXT_SECONDARY, width=4.2, key_size=19, value_size=19),
            small_field("message", "add search", color=GIT_HEAD, width=4.2, key_size=19, value_size=19),
        ...
        machine_pulse = pulse_ring(machine.get_center(), GIT_HEAD, radius=1.05)

        changed_message = small_field("message", "add Search", color=GIT_CONFLICT, width=4.2, key_size=19, value_size=19)
        ...
        note = label("任一字段改变，commit 身份就变。", size=32, color=TEXT_PRIMARY, weight="BOLD")
        ...
        self.play(FadeIn(machine_pulse, scale=0.6), machine.animate.set_stroke(color=GIT_HEAD, width=8), run_time=0.8)
        self.play(FadeOut(machine_pulse, scale=1.45), run_time=0.35)
        self.wait(1.45)
