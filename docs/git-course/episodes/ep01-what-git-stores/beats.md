...
- 0-2s：全屏石白背景，大标题 `Git 到底记录什么？` 独占画面。
- 2-3s：标题完全淡出，不保留顶部小标题或副标题。
- 3-9s：正式内容入场，三张极简文件卡依次出现：`project`、`project-final`、`project-final-2`，表达“误以为 Git 只是保存文件版本”。
- 9-12s：文件卡收束成一个问题，底部只保留一句短问句。
