...
- 概念切换、命令切换、列表结束转结论时，不能只依赖换行或段落空行。TTS 可能把它们读成连续句，必须在文稿里显式加入 `<#0.3#>` 到 `<#0.45#>` 的停顿。
- SRT 相邻 cue 的间隔低于约 `0.2s` 时必须人工判断：列表内部或一句话的自然承接可以保留；如果是“上一层意思结束，下一层意思开始”，需要回到 `.txt` 补停顿并重新生成。
