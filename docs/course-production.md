...
字号统一按 `1920×1080` 设计坐标中的逻辑像素记录；4K 只把同一设计等比渲染为两倍物理像素，不把规范数值翻倍。课程 adapter 可以使用不同字体和层级，但不得低于以下语义下限：

- 正文、结论、状态说明等主要教学文字不小于 `24px`；空间不足时先精简文案、增加容器或拆分镜头，不得靠缩小字号塞入。
- 图节点、branch/ref、文件名、卡片 eyebrow 和其他承担理解作用的教学标签不小于 `22px`。
- 代码与终端中的必要输出不小于 `20px`；主要命令和关键结果应使用 `22px` 以上。
- 解说字幕不小于 `31px`，并继续遵守课程自己的行数和安全区规则。
- `16–18px` 只允许用于章节进度、时间戳、非必要 metadata 和品牌微文字等 chrome。删除这些文字后不应影响观众理解当前教学动作。
- 顺序已经由从左到右、时间进入或箭头明确表达时，不再增加孤立的小号 `1 / 2 / 3 / 4`；编号确有教学意义时，必须作为不小于 `22px` 的完整视觉组呈现。

共享 token 只能按上述语义使用，不能把面向 chrome 的小字号 token 用在教学内容上。审查时必须在编码后的 1080p MP4 中确认实际可读性；4K 抽帧缩放后的“看起来够大”不能替代 1080p 基线检查。
