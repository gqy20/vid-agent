...
const arrow = ({x1, x2, y, label, color = INK}) => `
  <text x="${(x1 + x2) / 2}" y="${y - 30}" text-anchor="middle" font-family="${FONT.mono}" font-size="28" font-weight="820" fill="${color}">${esc(label)}</text>
...
const codeLine = (x, y, w, color = 'rgba(23,33,31,0.18)') =>
  `<rect x="${x}" y="${y}" width="${w}" height="10" rx="3" fill="${color}"/>`;
...
const body = `
  ...
  <g transform="translate(160 460)">
    ...
    <g transform="translate(40 100)">
      <rect width="320" height="230" rx="10" fill="#fff" stroke="rgba(23,33,31,0.18)" stroke-width="2"/>
      <circle cx="24" cy="26" r="7" fill="${TOMATO}"/>
      <circle cx="46" cy="26" r="7" fill="${MUSTARD}"/>
      <circle cx="68" cy="26" r="7" fill="${TEAL}"/>
      <text x="92" y="32" font-family="${FONT.mono}" font-size="18" font-weight="820" fill="${MUTE}">README.md</text>
      ${codeLine(28, 74, 250)}
      ${codeLine(28, 100, 200)}
      ${codeLine(28, 126, 230)}
  ...
  <g transform="translate(760 460)">
    ...
    <g transform="translate(80 108)">
      ...
      <text x="120" y="42" text-anchor="middle" font-family="${FONT.mono}" font-size="22" font-weight="840" fill="${INK}">snapshot</text>
      ...
      <text x="120" y="194" text-anchor="middle" font-family="${FONT.mono}" font-size="15" font-weight="780" fill="${MUTE}">staged for next commit</text>
    </g>
  ...
  <g transform="translate(1360 460)">
    ...
    <g transform="translate(40 0)">
      ...
      <text x="60" y="262" text-anchor="middle" font-family="${FONT.mono}" font-size="22" font-weight="840" fill="${INK}">C0</text>
      <text x="170" y="262" text-anchor="middle" font-family="${FONT.mono}" font-size="22" font-weight="840" fill="${INK}">C1</text>
      <text x="280" y="262" text-anchor="middle" font-family="${FONT.mono}" font-size="22" font-weight="840" fill="${INK}">C2</text>
      ...
      <text x="280" y="125" text-anchor="middle" font-family="${FONT.mono}" font-size="22" font-weight="880" fill="${INK}">main</text>
    </g>
