...
const Row: React.FC<{c: string; n: string; fail: string; rate: string; delay: number; warn?: boolean}> = ({c, n, fail, rate, delay, warn}) => (
  <Reveal delay={delay} y={8} style={{display: 'flex', color: warn ? C.warn : C.text, fontSize: 22}}>
...
export const SceneCmd: React.FC = () => (
  <Backdrop>
    <Terminal title="cc-insights — Bash 命令" width={1220} appear={0}>
      ...
      <div style={{height: 16}} />
      <Reveal delay={42} style={{color: C.dim, fontSize: 20}}>COMMAND · 最近 30 天 · 1,284 次 Bash 调用</Reveal>
      <div style={{height: 18}} />
      <Reveal delay={50} style={{display: 'flex', color: C.dim, fontSize: 20, borderBottom: `1px solid ${C.border}`, paddingBottom: 8}}>
      ...
      <Reveal delay={112} style={{color: C.text, fontSize: 21, paddingLeft: 24}}>rm -rf build/ && cp -r dist ...</Reveal>
      <Reveal delay={122} style={{color: C.text, fontSize: 21, paddingLeft: 24}}>curl -fsSL ... | bash  <span style={{color: C.red}}>（远程执行）</span></Reveal>
    </Terminal>
