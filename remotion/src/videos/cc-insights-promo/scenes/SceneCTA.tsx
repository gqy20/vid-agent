  ...
  return (
    <Backdrop>
      <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 38}}>
        ...
        <div style={{fontFamily: MONO, fontSize: 70, fontWeight: 700, color: C.white, textShadow: '0 0 26px rgba(86,212,196,0.7)'}}>
          cc-insights
        </div>
        <Reveal delay={22} style={{background: C.termBg, border: `1px solid ${C.border}`, borderRadius: 12, padding: '18px 34px', fontFamily: MONO, fontSize: 26}}>
          <span style={{color: C.green}}>$ </span>
          <span style={{color: C.white}}>cc-insights rec -p 7d</span>
          <span style={{color: C.dim}}>   # 一条命令，给你根因和下一步</span>
        </Reveal>
        <Reveal delay={38} style={{fontFamily: MONO, fontSize: 34, color: C.cyan}}>
          github.com/gqy20/cc-insights
        </Reveal>
        <Reveal delay={54} style={{fontFamily: SANS, fontSize: 26, color: C.dim}}>
