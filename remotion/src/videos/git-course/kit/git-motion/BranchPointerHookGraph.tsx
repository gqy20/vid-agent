...
export const BranchPointerHookGraph: React.FC = () => {
  ...
  const showLine01 = frame > seconds(2.75);
  const showLine12 = frame > seconds(3.85);
  const showLandingRing = landingRingOpacity > 0.01;
  const showMain = mainOpacity > 0.01;
  const showHead = headOpacity > 0.01;
  ...
  return (
    <svg data-audit-id="hook-main-graph" width="1120" height="520" viewBox="0 0 1120 520" style={{display: 'block', width: '100%', height: 'auto'}}>
      <g data-audit-id="hook-commit-chain">
        {showLine01 ? <line x1={150} y1="226" x2={line01End} y2="226" stroke={COLOR.git.graphLine} strokeWidth="11" strokeLinecap="round" /> : null}
        {showLine12 ? <line x1={410} y1="226" x2={line12End} y2="226" stroke={COLOR.git.graphLine} strokeWidth="11" strokeLinecap="round" /> : null}
        {showLandingRing ? <circle cx="670" cy="226" r={landingRingRadius} fill="none" stroke={COLOR.git.head} strokeWidth="7" opacity={landingRingOpacity} /> : null}
