...
const Takeaway:React.FC=()=>{const f=useCurrentFrame();const steps=[['1','fetch','先刷新参照'],['2','inspect','看清分叉'],['3','integrate','选择 merge / rebase'],['4','push','再请求更新远端']];return <AbsoluteFill style={{padding:'140px 210px',boxSizing:'border-box'}}><SceneTitle>拒绝不是故障，而是在保护服务器历史</SceneTitle><div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:26,marginTop:100}}>{steps.map((s,i)=><div key={s[1]} style={{padding:'30px 24px',background:COLOR.canvas.raised,borderRadius:22,textAlign:'center',opacity:appear(f,2+i*2.6)}}><div style={{fontFamily:FONT.mono,...TYPE.label,color:COLOR.text.tertiary}}>{s[0]}</div><div style={{fontFamily:FONT.mono,...TYPE.title,fontWeight:WEIGHT.bold,color:i===3?COLOR.git.feature:COLOR.git.main,margin:'12px 0'}}>{s[1]}</div><div style={{...TYPE.body,color:COLOR.text.secondary}}>{s[2]}</div></div>)}</div><NarrationSubtitle frame={f} cues={RUNTIME.captions('takeaway')} width={1320} bottom={64}/></AbsoluteFill>};
