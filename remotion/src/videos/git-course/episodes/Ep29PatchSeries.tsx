...
const SenderHistory=()=> <ModelScene title="先固定 Base..HEAD 的提交顺序" captions={R.captions('sender-history')}><FlowSteps steps={[{label:'0001',detail:'建立结构',tone:'feature'},{label:'0002',detail:'加入行为',tone:'feature'},{label:'0003',detail:'补充测试',tone:'feature'}]}/></ModelScene>;
// @git-course-scene sender-history:end
// @git-course-scene format-patch:start
const FormatPatch=()=> <Term scene="format-patch" id="ep29-format-patch" result={<><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>每条 Commit 对应一份 Patch Message</SceneTitle><FlowSteps steps={[{label:'C1',tone:'feature'},{label:'C2',tone:'feature'},{label:'C3',tone:'feature'}]}/></>}/>;
// @git-course-scene format-patch:end
// @git-course-scene inspect-message:start
const InspectMessage=()=> <Term scene="inspect-message" id="ep29-inspect-patch" result={<><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>一份 Patch 同时携带提交语义</SceneTitle><CompareCards cards={[{label:'From',detail:'作者信息',tone:'feature'},{label:'Subject',detail:'提交标题',tone:'head'},{label:'Body',detail:'为什么改变'},{label:'Diff',detail:'文件变化',tone:'main'}]}/></>}/>;
// @git-course-scene inspect-message:end
// @git-course-scene apply-series:start
const ApplySeries=()=> <Term scene="apply-series" id="ep29-apply-series" result={<><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>Git Am 按顺序重建三条 Commit</SceneTitle><FlowSteps steps={[{label:'0001',tone:'feature'},{label:'0002',tone:'feature'},{label:'0003',tone:'feature'},{label:'New OIDs',tone:'head'}]}/></>}/>;
