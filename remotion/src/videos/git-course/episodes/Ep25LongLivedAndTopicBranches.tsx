...
const Term=({scene,id,result}:{scene:Parameters<typeof R.captions>[0];id:string;result:React.ReactNode})=><TerminalThenModelScene cues={[cue(id)]} modelAtSeconds={10} model={result} captions={R.captions(scene)} auditIdPrefix={`ep25-${scene}`}/>;
...
const TopicStart=()=> <Term scene="topic-start" id="ep25-topic-start" result={<><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>一项工作，一条短期 Topic</SceneTitle><FlowSteps steps={[{label:'main',detail:'稳定入口'},{label:'topic/login',detail:'两次提交',tone:'feature'}]}/></>}/>;
...
const Integrate=()=> <Term scene="integrate" id="ep25-integrate" result={<><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>检查通过后，Topic 才进入 Main</SceneTitle><FlowSteps steps={[{label:'开发',tone:'feature'},{label:'测试',tone:'head'},{label:'整合'},{label:'main'}]}/></>}/>;
// @git-course-scene integrate:end
// @git-course-scene delete-ref:start
const DeleteRef=()=> <Term scene="delete-ref" id="ep25-delete" result={<><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>Ref 消失，Commit 仍由 Main 可达</SceneTitle><CompareCards cards={[{label:'topic/login',detail:'名字已删除',tone:'conflict'},{label:'C1 · C2',detail:'对象仍在历史中',tone:'feature'},{label:'main',detail:'仍可沿 parent 到达'}]}/></>}/>;
// @git-course-scene delete-ref:end
// @git-course-scene takeaway:start
const Takeaway=()=> <ModelScene title="先定义职责，再决定寿命" captions={R.captions('takeaway')}><FlowSteps steps={[{label:'隔离',detail:'一个主题',tone:'feature'},{label:'检查',detail:'可验证',tone:'head'},{label:'整合',detail:'进入稳定线'},{label:'删 Ref',detail:'对象仍可达'}]}/></ModelScene>;
