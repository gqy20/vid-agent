...
import {CenteredSceneBody,CourseLayout,EpisodeTitleCard,EpisodeTimeline,GitStatePanel,NarrationSubtitle,SceneTitle,TerminalThenModelScene,createEpisodeRuntime} from '../kit'; import {COLOR,WEIGHT} from '../palette'; import {seconds} from '../timeline'; import {TYPE} from '../typography';
...
const Term:React.FC<{scene:string;id:keyof typeof TERMINAL_RECORDINGS;d:number;children:React.ReactNode}>=({scene,id,d,children})=><TerminalThenModelScene cues={[cue(id,d)]} modelAtSeconds={d} model={children} captions={R.captions(scene as never)} auditIdPrefix={`ep18-${scene}-terminal`}/>;
const State=({clean=false}:{clean?:boolean})=><CenteredSceneBody width={1320}><GitStatePanel areas={[{id:'working-tree',title:'Working Tree',files:clean?['notes.tmp · untracked']:['app.js · unstaged','notes.tmp · untracked'],active:true},{id:'index',title:'Index',files:clean?['matches HEAD']:['config.js · staged'],active:!clean},{id:'repository',title:'Repository',files:['HEAD'],active:false}]} prominent/></CenteredSceneBody>;
...
const InspectEntry=()=>{const f=useCurrentFrame();return <AbsoluteFill><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>refs/stash 是本地可检查入口</SceneTitle><CenteredSceneBody width={1400} style={{padding:46,borderRadius:28,background:COLOR.canvas.raised}}><div style={{...TYPE.title,fontFamily:'monospace',fontWeight:WEIGHT.bold,color:COLOR.git.head}}>stash@&#123;0&#125;</div><div style={{...TYPE.body,marginTop:28,color:COLOR.text.secondary}}>On main: switch task · Working Tree + Index</div></CenteredSceneBody><NarrationSubtitle frame={f} cues={R.captions('inspect-entry')} width={1320} bottom={64}/></AbsoluteFill>};
...
const Lifecycle=()=>{const f=useCurrentFrame();return <AbsoluteFill><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>恢复内容，与删除 Entry 是两件事</SceneTitle><CenteredSceneBody width={1480} style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:34}}>{[['apply','恢复 · 保留'],['pop','成功恢复后移除'],['drop','只移除记录']].map((x,i)=><div key={x[0]} style={{padding:40,borderRadius:24,background:COLOR.canvas.raised,opacity:ap(f,2+i*3)}}><div style={{...TYPE.title,fontWeight:WEIGHT.bold,color:[COLOR.git.main,COLOR.git.feature,COLOR.git.conflict][i]}}>{x[0]}</div><div style={{...TYPE.body,marginTop:26}}>{x[1]}</div></div>)}</CenteredSceneBody><NarrationSubtitle frame={f} cues={R.captions('lifecycle')} width={1320} bottom={64}/></AbsoluteFill>};
// @git-course-scene lifecycle:end
// @git-course-scene takeaway:start
const Takeaway=()=>{const f=useCurrentFrame();return <AbsoluteFill><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>Stash 是本地短期现场，不是备份</SceneTitle><CenteredSceneBody width={1480} style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:28}}>{['status','push','show','apply --index'].map((x,i)=><div key={x} style={{padding:34,textAlign:'center',borderRadius:22,background:COLOR.canvas.raised,opacity:ap(f,2+i*2.4),...TYPE.title,fontSize:i===3?44:TYPE.title.fontSize,fontWeight:WEIGHT.bold,whiteSpace:'nowrap'}}>{x}</div>)}</CenteredSceneBody><NarrationSubtitle frame={f} cues={R.captions('takeaway')} width={1320} bottom={64}/></AbsoluteFill>};
