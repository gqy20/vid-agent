...
const PublishTopic=()=> <Term scene="publish-topic" id="ep27-publish-topic" at={10.5} result={<><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>贡献者只更新自己的 Topic Ref</SceneTitle><RepositoryTopology repositories={[{label:'Local Topic',ref:'C1 · C2',tone:'feature'},{label:'contributor.git',ref:'topic/login',tone:'feature'}]}/></>}/>;
// @git-course-scene publish-topic:end
// @git-course-scene maintainer-fetch:start
const MaintainerFetch=()=> <Term scene="maintainer-fetch" id="ep27-maintainer-fetch" at={12.3} result={<><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>Fetch 带来对象与可检查范围</SceneTitle><FlowSteps revealAtSeconds={12.3} steps={[{label:'Add Remote',tone:'head'},{label:'Fetch'},{label:'main..topic',tone:'feature'}]}/></>}/>;
// @git-course-scene maintainer-fetch:end
// @git-course-scene review-integrate:start
const ReviewIntegrate=()=> <Term scene="review-integrate" id="ep27-review-integrate" at={11} result={<><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>维护者检查后更新 Canonical</SceneTitle><FlowSteps revealAtSeconds={11} steps={[{label:'Inspect',tone:'head'},{label:'Test',tone:'head'},{label:'Integrate',tone:'feature'},{label:'Canonical'}]}/></>}/>;
// @git-course-scene review-integrate:end
// @git-course-scene takeaway:start
const Takeaway=()=> <ModelScene title="发布 Ref，交出可验证范围" captions={R.captions('takeaway')}><FlowSteps revealAtSeconds={.4} steps={[{label:'Publish',tone:'feature'},{label:'Fetch'},{label:'Review',tone:'head'},{label:'Accept'}]}/></ModelScene>;
