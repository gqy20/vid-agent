...
export {EP28_DURATION_IN_FRAMES,EP28_SCENES}; const R=createEpisodeRuntime(EP28_SCENES); const cue=(id:string,d=10)=>({id:id as keyof typeof TERMINAL_RECORDINGS,from:0,durationInFrames:seconds(d+.7),src:`git-course-lab/terminal/${id}.mp4`,holdFrameSrc:`git-course-lab/terminal/${id}-hold.png`,holdFromFrame:TERMINAL_RECORDINGS[id as keyof typeof TERMINAL_RECORDINGS].holdFromFrame}); const Term=({scene,id,result,at=10}:{scene:Parameters<typeof R.captions>[0];id:string;result:React.ReactNode;at?:number})=><TerminalThenModelScene cues={[cue(id,at)]} modelAtSeconds={at} model={result} captions={R.captions(scene)} auditIdPrefix={`ep28-${scene}`}/>;
...
const InspectDirty=()=> <Term scene="inspect-dirty" id="ep28-inspect-dirty" at={11} result={<><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>先移除不属于贡献的噪声</SceneTitle><CompareCards cards={[{label:'功能',detail:'本次需要的变化'},{label:'DEBUG',detail:'临时输出，移除',tone:'conflict'},{label:'Whitespace',detail:'diff --check 定位',tone:'head'}]}/></>}/>;
...
const VerifyIndex=()=> <Term scene="verify-index" id="ep28-verify-index" at={11} result={<><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>先核对 Index，再建立 Commit</SceneTitle><FlowSteps revealAtSeconds={11} steps={[{label:'Cached Diff',tone:'head'},{label:'Test'},{label:'Commit',tone:'feature'}]}/></>}/>;
// @git-course-scene verify-index:end
// @git-course-scene receiver-range:start
const ReceiverRange=()=> <Term scene="receiver-range" id="ep28-review-range" at={12.5} result={<><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>接收者看到的是 Upstream..HEAD</SceneTitle><FlowSteps revealAtSeconds={12.5} steps={[{label:'C1',detail:'功能',tone:'feature'},{label:'C2',detail:'测试'},{label:'Clean WT',detail:'无遗漏',tone:'head'}]}/></>}/>;
// @git-course-scene receiver-range:end
// @git-course-scene takeaway:start
const Takeaway=()=> <ModelScene title="清楚的范围，比统一模板更重要" captions={R.captions('takeaway')}><FlowSteps revealAtSeconds={.4} steps={[{label:'Scope',detail:'只有所需变化'},{label:'Intent',detail:'一条一个意图',tone:'feature'},{label:'Evidence',detail:'可独立验证',tone:'head'},{label:'Range',detail:'接收者视角'}]}/></ModelScene>;
