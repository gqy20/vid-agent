...
const Terminal:React.FC<{scene:string;cues:readonly ReturnType<typeof terminalCue>[]}>=({scene,cues})=>{const f=useCurrentFrame();return <AbsoluteFill><RecordedTerminalCueSequence cues={cues} rect={{x:250,y:132,width:1420,height:720}} auditIdPrefix={`ep15-${scene}-terminal`}/><NarrationSubtitle frame={f} cues={RUNTIME.captions(scene as never)} width={1320} bottom={64}/></AbsoluteFill>};
...
const CreateConflict:React.FC=()=> <Terminal scene="create-conflict" cues={[terminalCue('ep15-merge-conflict',0,22)]}/>;
// @git-course-scene create-conflict:end
// @git-course-scene index-stages:start
const IndexStages:React.FC=()=>{const f=useCurrentFrame();return <AbsoluteFill style={{padding:'112px 170px',boxSizing:'border-box'}}><SceneTitle>冲突时，Index 暂存三份输入</SceneTitle><ThreeStages frame={f}/><div style={{textAlign:'center',marginTop:42,...TYPE.subtitle,color:COLOR.text.secondary,opacity:appear(f,11)}}>它们是合并输入，不是三份 Working Tree 文件</div><NarrationSubtitle frame={f} cues={RUNTIME.captions('index-stages')} width={1320} bottom={64}/></AbsoluteFill>};
// @git-course-scene index-stages:end
// @git-course-scene inspect-stages:start
const InspectStages:React.FC=()=> <Terminal scene="inspect-stages" cues={[terminalCue('ep15-ls-files-unmerged',0,6),terminalCue('ep15-show-base',6,5),terminalCue('ep15-show-ours',11,5),terminalCue('ep15-show-theirs',16,5)]}/>;
// @git-course-scene inspect-stages:end
// @git-course-scene resolve-working-tree:start
const ResolveWorkingTree:React.FC=()=>{const f=useCurrentFrame();return <AbsoluteFill style={{padding:'112px 250px',boxSizing:'border-box'}}><SceneTitle>先在 Working Tree 写出最终结果</SceneTitle><div style={{margin:'74px auto 0',maxWidth:1080,borderRadius:24,background:COLOR.canvas.raised,padding:'38px 46px'}}><div style={{fontFamily:FONT.mono,...TYPE.label,fontSize:24,color:COLOR.text.tertiary}}>app.js</div><div style={{fontFamily:FONT.mono,fontSize:42,lineHeight:1.5,fontWeight:WEIGHT.bold,marginTop:26}}><span style={{color:COLOR.git.workingTree}}>const</span> title = <span style={{color:COLOR.git.feature}}>&quot;resolved&quot;</span>;</div></div><div style={{textAlign:'center',marginTop:48,...TYPE.title,fontWeight:WEIGHT.bold,color:COLOR.git.conflict,opacity:appear(f,8)}}>删掉标记 ≠ 已告诉 Index 冲突解决</div><NarrationSubtitle frame={f} cues={RUNTIME.captions('resolve-working-tree')} width={1320} bottom={64}/></AbsoluteFill>};
// @git-course-scene resolve-working-tree:end
// @git-course-scene stage-zero:start
const StageZero:React.FC=()=>{const f=useCurrentFrame();return <AbsoluteFill><RecordedTerminalCueSequence cues={[terminalCue('ep15-add-resolved',0,6),terminalCue('ep15-stage-zero',6,6)]} rect={{x:250,y:132,width:1420,height:720}} auditIdPrefix="ep15-stage-zero-terminal"/><div style={{position:'absolute',left:550,right:550,top:250,display:'flex',justifyContent:'center',opacity:appear(f,12.1)}}><ThreeStages frame={Math.max(0,f-seconds(12))} stageZero/></div><NarrationSubtitle frame={f} cues={RUNTIME.captions('stage-zero')} width={1320} bottom={64}/></AbsoluteFill>};
...
const Takeaway:React.FC=()=>{const f=useCurrentFrame();const rows=[['unmerged','stage 1 / 2 / 3'],['resolve','编辑 Working Tree'],['git add','写入 stage 0'],['continue','完成 merge / rebase']];return <AbsoluteFill style={{padding:'130px 280px',boxSizing:'border-box'}}><SceneTitle>把冲突看成 Index 状态机</SceneTitle><div style={{display:'grid',gap:20,marginTop:40}}>{rows.map((r,i)=><div key={r[0]} style={{display:'grid',gridTemplateColumns:'290px 1fr',padding:'22px 28px',borderRadius:18,background:COLOR.canvas.raised,opacity:appear(f,2+i*2.5)}}><span style={{fontFamily:FONT.mono,...TYPE.title,fontWeight:WEIGHT.bold,color:i<2?COLOR.git.conflict:COLOR.git.index}}>{r[0]}</span><span style={{...TYPE.subtitle,textAlign:'right'}}>{r[1]}</span></div>)}</div><NarrationSubtitle frame={f} cues={RUNTIME.captions('takeaway')} width={1320} bottom={64}/></AbsoluteFill>};
