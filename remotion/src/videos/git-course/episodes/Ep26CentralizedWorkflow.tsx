...
export {EP26_DURATION_IN_FRAMES,EP26_SCENES}; const R=createEpisodeRuntime(EP26_SCENES); const cue=(id:string,d=10)=>({id:id as keyof typeof TERMINAL_RECORDINGS,from:0,durationInFrames:seconds(d+.7),src:`git-course-lab/terminal/${id}.mp4`,holdFrameSrc:`git-course-lab/terminal/${id}-hold.png`,holdFromFrame:TERMINAL_RECORDINGS[id as keyof typeof TERMINAL_RECORDINGS].holdFromFrame}); const Term=({scene,id,result}:{scene:Parameters<typeof R.captions>[0];id:string;result:React.ReactNode})=><TerminalThenModelScene cues={[cue(id)]} modelAtSeconds={10} model={result} captions={R.captions(scene)} auditIdPrefix={`ep26-${scene}`}/>;
...
const BobRejected=()=> <Term scene="bob-rejected" id="ep26-bob-reject" result={<><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>Bob 的旧历史不能覆盖已发布历史</SceneTitle><FlowSteps steps={[{label:'Bob B1',tone:'feature'},{label:'Push',tone:'head'},{label:'Rejected',tone:'conflict'}]}/></>}/>;
// @git-course-scene bob-rejected:end
// @git-course-scene bob-integrates:start
const BobIntegrates=()=> <Term scene="bob-integrates" id="ep26-bob-integrate" result={<><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>Fetch 后在本地整合，再 Push</SceneTitle><FlowSteps steps={[{label:'Fetch'},{label:'Local Merge',tone:'feature'},{label:'Test',tone:'head'},{label:'Push'}]}/></>}/>;
