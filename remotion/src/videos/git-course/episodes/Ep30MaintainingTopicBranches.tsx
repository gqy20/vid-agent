...
export {EP30_DURATION_IN_FRAMES,EP30_SCENES}; const R=createEpisodeRuntime(EP30_SCENES); const cue=(id:string,d=10)=>({id:id as keyof typeof TERMINAL_RECORDINGS,from:0,durationInFrames:seconds(d+.7),src:`git-course-lab/terminal/${id}.mp4`,holdFrameSrc:`git-course-lab/terminal/${id}-hold.png`,holdFromFrame:TERMINAL_RECORDINGS[id as keyof typeof TERMINAL_RECORDINGS].holdFromFrame}); const Term=({scene,id,result,at=10}:{scene:Parameters<typeof R.captions>[0];id:string;result:React.ReactNode;at?:number})=><TerminalThenModelScene cues={[cue(id,at)]} modelAtSeconds={at} model={result} captions={R.captions(scene)} auditIdPrefix={`ep30-${scene}`}/>;
...
const UpdatedTopic=()=> <ModelScene title="修正版只更新 Bob 的评估路径" captions={R.captions('updated-topic')}><FlowSteps revealAtSeconds={.4} steps={[{label:'Bob v1',detail:'test failed',tone:'conflict'},{label:'Fetch v2',detail:'更新观察 ref',tone:'head'},{label:'Review',detail:'重新验证',tone:'feature'}]}/></ModelScene>;
// @git-course-scene updated-topic:end
// @git-course-scene throwaway-integration:start
const ThrowawayIntegration=()=> <Term scene="throwaway-integration" id="ep30-integration-test" result={<><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>临时 Integration 验证组合结果</SceneTitle><FlowSteps revealAtSeconds={10} steps={[{label:'Topic A',tone:'feature'},{label:'Topic B',tone:'feature'},{label:'Combine',tone:'head'},{label:'Main'}]}/></>}/>;
// @git-course-scene throwaway-integration:end
// @git-course-scene takeaway:start
const Takeaway=()=> <ModelScene title="隔离评估，临时组合，最后接纳" captions={R.captions('takeaway')}><FlowSteps revealAtSeconds={.4} steps={[{label:'Fetch'},{label:'Isolate',tone:'feature'},{label:'Combine',tone:'head'},{label:'Accept'}]}/></ModelScene>;
