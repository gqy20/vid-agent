...
export {EP31_DURATION_IN_FRAMES,EP31_SCENES}; const R=createEpisodeRuntime(EP31_SCENES); const cue=(id:string,d=10)=>({id:id as keyof typeof TERMINAL_RECORDINGS,from:0,durationInFrames:seconds(d+.7),src:`git-course-lab/terminal/${id}.mp4`,holdFrameSrc:`git-course-lab/terminal/${id}-hold.png`,holdFromFrame:TERMINAL_RECORDINGS[id as keyof typeof TERMINAL_RECORDINGS].holdFromFrame}); const Term=({scene,id,result}:{scene:Parameters<typeof R.captions>[0];id:string;result:React.ReactNode})=><TerminalThenModelScene cues={[cue(id)]} modelAtSeconds={10} model={result} captions={R.captions(scene)} auditIdPrefix={`ep31-${scene}`}/>;
...
const Hotfix=()=> <Term scene="hotfix" id="ep31-hotfix" result={<><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>修复从最老受支持位置开始</SceneTitle><FlowSteps steps={[{label:'v1.0.0',tone:'head'},{label:'maint/v1',tone:'feature'},{label:'hotfix/login',tone:'conflict'}]}/></>}/>;
// @git-course-scene hotfix:end
// @git-course-scene maintenance-release:start
const MaintenanceRelease=()=> <Term scene="maintenance-release" id="ep31-maint-release" result={<><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>维护线通过后发布 V1.0.1</SceneTitle><FlowSteps steps={[{label:'Fix',tone:'conflict'},{label:'Test',tone:'head'},{label:'maint/v1',tone:'feature'},{label:'v1.0.1',tone:'head'}]}/></>}/>;
// @git-course-scene maintenance-release:end
// @git-course-scene merge-up:start
const MergeUp=()=> <Term scene="merge-up" id="ep31-merge-up" result={<><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>修复再沿维护线向上进入 Main</SceneTitle><FlowSteps steps={[{label:'maint/v1',tone:'feature'},{label:'Merge Up',tone:'head'},{label:'main'},{label:'Test'}]}/></>}/>;
// @git-course-scene merge-up:end
// @git-course-scene takeaway:start
const Takeaway=()=> <ModelScene title="先修最老支持线，再向上整合" captions={R.captions('takeaway')}><FlowSteps steps={[{label:'Oldest',detail:'最老支持线',tone:'feature'},{label:'Fix',detail:'修复与验证',tone:'conflict'},{label:'Release',detail:'维护版本',tone:'head'},{label:'Merge Up',detail:'进入更新线'}]}/></ModelScene>;
