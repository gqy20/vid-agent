...
import {CourseCommitNode,CourseGraphEdge,CourseLayout,EpisodeTitleCard,EpisodeTimeline,GitStatePanel,NarrationSubtitle,RecordedTerminalCueSequence,SceneTitle,createEpisodeRuntime,courseCommitAnchor} from '../kit';
...
const Terminal:React.FC<{scene:string;id:keyof typeof TERMINAL_RECORDINGS;duration:number;children?:React.ReactNode}>=({scene,id,duration,children})=>{const f=useCurrentFrame();return <AbsoluteFill><RecordedTerminalCueSequence cues={[cue(id,duration)]} rect={{x:250,y:132,width:1420,height:720}} auditIdPrefix={`ep17-${scene}-terminal`}/>{children?<AbsoluteFill style={{background:COLOR.canvas.base,opacity:appear(f,duration)}}>{children}</AbsoluteFill>:null}<NarrationSubtitle frame={f} cues={R.captions(scene as never)} width={1320} bottom={64}/></AbsoluteFill>};
const SplitModel:React.FC=()=> <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:44,width:1280,margin:'220px auto 0'}}>{[['bug fix','Math.max(total, 1)',COLOR.git.workingTree],['copy edit','checkout ready',COLOR.git.feature]].map(x=><div key={x[0]} style={{padding:38,borderRadius:24,background:COLOR.canvas.raised,border:`3px solid ${x[2]}`}}><div style={{...TYPE.label,fontFamily:FONT.mono,color:x[2]}}>{x[0]}</div><div style={{...TYPE.title,fontFamily:FONT.mono,fontWeight:WEIGHT.bold,marginTop:28}}>{x[1]}</div></div>)}</div>;
...
const ChoosePatch=()=> <Terminal scene="choose-patch" id="ep17-add-p" duration={12}><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>y 写入 Index，n 留在 Working Tree</SceneTitle><div style={{position:'absolute',left:300,top:270,width:1320}}><GitStatePanel areas={[{id:'working-tree',title:'Working Tree',files:['copy edit'],active:true},{id:'index',title:'Index',files:['bug fix'],active:true},{id:'repository',title:'Repository',files:['HEAD'],active:false}]} prominent/></div></Terminal>;
// @git-course-scene choose-patch:end
// @git-course-scene verify-index:start
const VerifyIndex=()=> <Terminal scene="verify-index" id="ep17-verify" duration={10}><SceneTitle style={{position:'absolute',top:112,left:180,right:180}}>从两侧验证，而不是相信一次选择</SceneTitle><div style={{position:'absolute',left:300,top:270,width:1320}}><GitStatePanel areas={[{id:'working-tree',title:'Working Tree',files:['app.js · copy edit'],active:true},{id:'index',title:'Index',files:['app.js · bug fix'],active:true},{id:'repository',title:'Repository',files:['HEAD'],active:false}]} prominent/></div></Terminal>;
...
const Takeaway=()=>{const f=useCurrentFrame();return <AbsoluteFill style={{padding:'130px 220px',boxSizing:'border-box'}}><SceneTitle>先定义意图，再操作 Index</SceneTitle><div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:36,marginTop:110}}>{[['看','diff 识别 hunk'],['选','add -p 回答'],['验','cached / working']].map((x,i)=><div key={x[0]} style={{padding:38,textAlign:'center',borderRadius:24,background:COLOR.canvas.raised,opacity:appear(f,2+i*3)}}><div style={{...TYPE.hero,fontWeight:WEIGHT.bold,color:[COLOR.git.main,COLOR.git.head,COLOR.git.feature][i]}}>{x[0]}</div><div style={{...TYPE.body,fontWeight:WEIGHT.bold,marginTop:24}}>{x[1]}</div></div>)}</div><NarrationSubtitle frame={f} cues={R.captions('takeaway')} width={1320} bottom={64}/></AbsoluteFill>};
