...
const Term:React.FC<{scene:string;id:keyof typeof TERMINAL_RECORDINGS;duration:number;result:React.ReactNode}>=({scene,id,duration,result})=>{const f=useCurrentFrame();return <AbsoluteFill><RecordedTerminalCueSequence cues={[cue(id,duration)]} rect={{x:250,y:132,width:1420,height:720}} auditIdPrefix={`ep21-${scene}-terminal`}/><AbsoluteFill style={{background:COLOR.canvas.base,opacity:appear(f,duration)}}>{result}</AbsoluteFill><NarrationSubtitle frame={f} cues={R.captions(scene as never)} width={1320} bottom={64}/></AbsoluteFill>};
const Cards:React.FC<{items:Array<[string,string,string]>}>=({items})=>{const f=useCurrentFrame();return <div style={{display:'grid',gridTemplateColumns:`repeat(${items.length},1fr)`,gap:32,width:1500,margin:'330px auto 0'}}>{items.map(([label,value,color],i)=><div key={label} style={{padding:'38px 28px',borderRadius:24,background:COLOR.canvas.raised,border:`4px solid ${color}`,opacity:appear(f,1+i*1.3),textAlign:'center'}}><div style={{...TYPE.label,color}}>{label}</div><div style={{...TYPE.title,fontFamily:FONT.mono,fontWeight:WEIGHT.bold,marginTop:24}}>{value}</div></div>)}</div>};
